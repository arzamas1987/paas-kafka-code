...
cat << 'PYEOF' > pizza_app/**init**.py
from .config import kafka_settings

**all** = ["kafka_settings"]
PYEOF

cat << 'PYEOF' > pizza_app/models/**init**.py
...
cat << 'PYEOF' > pizza_app/generator/**init**.py

# Generators for synthetic pizza data (menu, addresses, orders)

PYEOF

cat << 'PYEOF' > pizza_app/services/**init**.py
...
if order_type == "delivery":
    ...
    # onsite: no address, table number optional
...
# services/stage_order_received.py
...
# chapter11 producer/consumer wrappers
